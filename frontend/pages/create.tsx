...
const quizSchema = z.object({
  ...
  questions: z
    ...
    .min(1, "A quiz must contain at least 1 question"),
});
...
export default function CreateQuiz() {
  ...
  const handleTypeChange = (
    index: number,
    newType: "BOOLEAN" | "INPUT" | "CHECKBOX",
  ) => {
  ...
  const handleRemoveCheckboxOption = (qIndex: number, optIndex: number) => {
    ...
    const updatedCorrect = currentCorrect.filter(
      (ans) => ans !== removedOptionText,
    );
    setValue(
      `questions.${qIndex}.correctAnswers`,
      updatedCorrect.length > 0 ? updatedCorrect : [currentOptions[0] || ""],
    );
  };

  const handleToggleCheckboxCorrectAnswer = (
    qIndex: number,
    optionText: string,
  ) => {
    const currentCorrect = [...(watchedQuestions[qIndex].correctAnswers || [])];
        ...
        <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
          <div className={styles.formHeader}>
            <div>
              ...
              <p className={styles.pageSubtitle}>
                Define your quiz schema, add questions and configure
                validations.
              </p>
            </div>
            <button
            ...
            >
              {submitting ? (
                <CircleDot className={styles.spinner} size={18} />
              ) : (
                <Save size={18} />
              )}
                    ...
                    <div className={styles.typeDetailsBox}>
                      ...
                      <div className={styles.booleanOptions}>
                        {["True", "False"].map((option) => {
                          const isCorrect =
                            watchedQuestions[qIndex]?.correctAnswers?.includes(
                              option,
                            );
                          return (
                            <button
                              ...
                              onClick={() =>
                                setValue(`questions.${qIndex}.correctAnswers`, [
                                  option,
                                ])
                              }
                            >
                    ...
                    <div className={styles.typeDetailsBox}>
                      ...
                      >
                        <input
                          ...
                          value={
                            watchedQuestions[qIndex]?.correctAnswers?.[0] || ""
                          }
                          onChange={(e) =>
                            setValue(`questions.${qIndex}.correctAnswers`, [
                              e.target.value,
                        ...
                        {qErrors?.correctAnswers && (
                          <p className={styles.inputError}>
                            {qErrors.correctAnswers.message}
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  {qType === "CHECKBOX" && (
                    <div className={styles.typeDetailsBox}>
                      ...
                      <div className={styles.optionsList}>
                        {(watchedQuestions[qIndex]?.options || []).map(
                          (optionText, optIndex) => {
                            const isCorrect =
                              watchedQuestions[
                                qIndex
                              ]?.correctAnswers?.includes(optionText);
                            return (
                              <div
                                key={optIndex}
                              ...
                              >
                                <button
                                  ...
                                  onClick={() =>
                                    handleToggleCheckboxCorrectAnswer(
                                      qIndex,
                                      optionText,
                                    )
                                  }
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const currentOptions = [
                                      ...(watchedQuestions[qIndex].options ||
                                        []),
                                    ];
                                    ...
                                    const currentCorrect = [
                                      ...(watchedQuestions[qIndex]
                                        .correctAnswers || []),
                                    ];
                                    const correctIndex =
                                      currentCorrect.indexOf(oldVal);
                                    if (correctIndex !== -1) {
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    handleRemoveCheckboxOption(qIndex, optIndex)
                                  }
                                  title="Remove Option"
                                  disabled={
                                    (watchedQuestions[qIndex]?.options || [])
          ...
          <div className={styles.formFooterSubmit}>
            ...
            >
              ...
              <span>
                {submitting ? "Creating Quiz..." : "Create Quiz Blueprint"}
              </span>
            </button>
          </div>
        </form>
